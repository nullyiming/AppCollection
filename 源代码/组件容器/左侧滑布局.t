类 左侧滑布局 : 组件容器
	...
	事件 左侧滑布局:创建完毕()
		...
		事件 监听回调:已初始化() : 逻辑型
			...
			int[][] colors = {{0xFF8ED1FC, 0xFF53C4D9, 0xFF319795},{0xFFFAD0C4, 0xFFF08080, 0xFFE6B8AF},{0xFF56CCF2, 0xFF2F80ED, 0xFF1C3F95}};
			float[] positions = null;
			android.graphics.LinearGradient grad = new android.graphics.LinearGradient(0, 0, viewWidth, viewHeight, colors[0], positions, android.graphics.Shader.TileMode.REPEAT);
			#文本框1.getView().getPaint().setShader(grad);
			@end
			返回 假
		结束 事件
		
		置圆角背景(线性布局2, 白色, DP到PX(取安卓环境(), 16), DP到PX(取安卓环境(), 16), 0, 0)
	结束 事件
