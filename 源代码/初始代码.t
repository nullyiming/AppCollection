...
类 启动窗口 : 窗口
	...
	变量 相对布局1 : 相对布局
	@布局配置([[父布局=相对布局1,ID=3,宽度=-1,高度DP=64,阴影=12,背景颜色=-592138]])
	变量 线性布局2 : 线性布局
	@布局配置([[父布局=线性布局2,宽度DP=64,高度DP=64]])
	变量 线性布局6 : 线性布局
	@布局配置([[父布局=线性布局6,宽度=-1,高度=-1,@外边距DP=16,图片资源="../../资源/menu.xml"]])
	变量 图片框1 : 圆角图片框
	@布局配置([[父布局=线性布局2,高度DP=64,内边距DP=8,对齐方式=16,字体大小=18,字体颜色=-16777216,宽度=-1,@权重=1.0,单行显示=真,省略显示=2]])
	变量 文本框1 : 文本框
	@布局配置([[父布局=线性布局2,宽度DP=64,高度DP=64]])
	变量 线性布局3 : 线性布局
	...
	@布局配置([[父布局=线性布局2,高度DP=64,宽度DP=64]])
	变量 线性布局5 : 线性布局
	@布局配置([[父布局=线性布局5,宽度=-1,高度=-1,@外边距DP=16,图片资源="../../资源/dots_vertical.xml"]])
	变量 图片框2 : 圆角图片框
	@布局配置([[父布局=相对布局1,ID=2,@位于某组件之上=1,@位于某组件之下=3]])
	变量 分页布局1 : 分页布局
	@布局配置([[父布局=相对布局1,宽度=-1,@位于布局底部=真,ID=1,高度=-2,阴影=12,背景颜色=-592138]])
	变量 线性布局1 : 线性布局
	...
	方法 加载底栏事件()
		...
		页面组.添加成员("主页")
		页面组.添加成员("测试")
		事件 底栏:加载布局(索引 : 整数,项目布局 : 组件容器) : 组件容器
			变量 布局 : 图标文本底栏项目?
			如果 项目布局 == 空 则
				布局 = 创建 图标文本底栏项目
			否则
	...
	*/
	...
	方法 加载主页列表类型事件()
		事件 主页列表类型.自定义宫格列表框1:加载布局(索引 : 整数,项目布局 : 组件容器) : 组件容器
			变量 布局 : 主页列表类型项目布局?
			如果 项目布局 == 空 则
				布局 = 创建 主页列表类型项目布局
			否则
			...
			结束 如果
			布局.文本框1.内容 = "应用" + (索引 + 1)
			布局.文本框2.内容 = "应用" + (索引 + 1) + "的介绍:::::::::::::::::::::::::::::::::::::"
			返回 布局
		结束 事件
		主页列表类型.自定义宫格列表框1.更新项目(100)
	结束 方法
